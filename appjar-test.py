...
if username_field not in colums:
    try:
        c.execute('ALTER TABLE ' + table_accounts + ' ADD COLUMN ' + username_field + ' TEXT')
    except:
        c.execute('CREATE TABLE {tn} ({nf} {ft})' .format(tn=table_accounts, nf=username_field, ft=field_type))
if salt_field not in colums:
    c.execute('ALTER TABLE ' + table_accounts + ' ADD COLUMN ' + salt_field + ' TEXT')
if hash_field not in colums:
    c.execute('ALTER TABLE ' + table_accounts + ' ADD COLUMN ' + hash_field + ' TEXT')
# ................................
...
def add_account(username, salt, hashed_password):
    c.execute("INSERT INTO %s VALUES('%s', '%s', '%s')" % (table_accounts, username, salt, hashed_password))
    conn.commit()
...
def press(button):
    if button == "Cancel":
        ...
        # print("User:", usr, "Pass:", pwd, "Hashed Password:", hashed_password)
    else:
